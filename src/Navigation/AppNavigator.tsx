  ...
  return (
    <NavigationContainer>
      <Stack.Navigator initialRouteName="GeneralOverview">
        <Stack.Screen name="GeneralOverview" options={{ headerShown: false }}>
          {(props) => <GeneralOverview {...props} events={events} />}
        </Stack.Screen>

        <Stack.Screen name="AffectedLocation" options={{ headerShown: false }}>
          {(props) => (
            <AffectedLocation {...props} nearbyEvents={nearbyEvents} />
          )}
        </Stack.Screen>
        <Stack.Screen
